    ...
    <div className="min-h-screen bg-white text-neutral-900">
      ...
      <section className="border-b border-neutral-200 bg-neutral-50">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-4 py-10 sm:px-6 md:flex-row md:py-12">
          <div className="text-center md:text-left">
            <p className="text-xs font-medium uppercase tracking-wider text-neutral-500">Upcoming Sale</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">Summer Sale — Up to 40% Off</h1>
            <p className="mt-2 text-sm text-neutral-600">Sitewide discounts on selected items. Don't miss out.</p>
          </div>
          <div className="flex gap-2 sm:gap-3">
            {[{ label: 'Days', value: d }, { label: 'Hours', value: h }, { label: 'Min', value: m }, { label: 'Sec', value: s }].map((t) => (
              <div key={t.label} className="min-w-[60px] rounded-md border border-neutral-200 bg-white px-3 py-2 text-center sm:min-w-[68px]">
                <div className="text-xl font-semibold tabular-nums sm:text-2xl">{pad(t.value)}</div>
                <div className="mt-0.5 text-[10px] uppercase tracking-wider text-neutral-500">{t.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-6 md:grid-cols-3 lg:grid-cols-4">
          {products.map((p) => (
            <Link key={p.id} to="/products/$productId" params={{ productId: String(p.id) }} className="group">
              <article>
