...
type OrderItem = { id: number; name: string; size: number; quantity: number; price: number };
  ...
  const handlePay = async () => {
    ...
    try {
      const orderRes = await apiFetch('/api/orders', {
        method: 'POST',
        body: JSON.stringify({ items: mockOrder.map((i) => ({ productId: i.id, size: i.size, quantity: i.quantity })) }),
      });
      const orderData = await orderRes.json() as { orderId: string; status: string } | { error: string };
      if (!orderRes.ok || !('orderId' in orderData)) {
        setStatus('failed');
        setFailReason('error' in orderData ? orderData.error : '주문 생성 실패');
        return;
      }
      ...
      const payData = await payRes.json() as { status: 'PAID' } | { status: 'FAILED'; failedReason: string } | { error: string };
      if (!payRes.ok || 'error' in payData) {
        setStatus('failed');
        setFailReason('error' in payData ? payData.error : '결제 처리 오류');
        return;
      }
      if (payData.status === 'PAID') setStatus('success');
      else { setStatus('failed'); setFailReason(payData.failedReason); }
    } catch (err) {
      setStatus('failed');
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        ...
        {status === 'success' ? (
          <div className="rounded-lg border border-green-200 bg-green-50 p-8 text-center">
            ...
            <Link to="/" className="mt-6 inline-block rounded-md bg-neutral-900 px-5 py-2 text-sm text-white">홈으로 돌아가기</Link>
          </div>
        ) : (
                ...
                {mockOrder.map((item) => (
                  <li key={item.id} className="flex items-center justify-between py-3 text-sm">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-neutral-500">사이즈 {item.size} · 수량 {item.quantity}개</p>
                    </div>
