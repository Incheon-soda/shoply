...
## 15. 외부에서 사이트 접속 안 됨 — EIP DNAT 규칙 누락/IP불일치 (doc13 14-5)

### 증상 A — Connection refused (즉시, ~수십ms)
- Mac에서 `curl -v http://<EIP>` → `Connection refused` (timeout 아니고 즉시 RST)
- `kubectl get ingress -n shoply`, `kubectl get svc -n ingress-nginx ingress-nginx-controller`는 정상(EXTERNAL-IP = VIP)

### 증상 B — 502 / 타임아웃
- 클러스터 내부에서 ingress 직접 호출(`curl -H "Host: shoply.example.com" http://<VIP>`)은 되는데 **EIP로는 안 됨**

### 원인
호스트 EC2의 `PREROUTING` DNAT 규칙(doc13 14-5: `-d $HOST_IP --dport 80/443 -j DNAT --to $VIP`)이:
- **누락**됐거나
- `$HOST_IP`가 **이전 재구축 때의 사설IP**로 박혀있어 실제 IP와 불일치 (EC2를 재생성/재시작하면 사설IP가 바뀜)

→ "Connection refused"가 뜨는 건, EIP→사설IP NAT까지는 호스트에 도착했는데 **DNAT 규칙의 `-d` 조건이 안 맞아 매칭 안 되고**, 호스트 자체는 80/443을 안 듣고 있어서 커널이 즉시 RST를 보내기 때문.

### 진단 (호스트 EC2)
```bash
hostname -I                                           # ① 현재 진짜 사설IP 확인 (hostname의 ip-x-x-x-x와 다를 수 있음!)
sudo iptables -t nat -L PREROUTING -n -v --line-numbers | grep -E "dpt:80|dpt:443"
#   pkts가 0이거나, destination이 ①의 IP와 다르면 원인 확정
cat /proc/sys/net/ipv4/ip_forward                     # 1 이어야 함
```

### 해결
```bash
# 옛 IP로 걸린 규칙 삭제 (큰 번호부터)
sudo iptables -t nat -D PREROUTING <번호2>
sudo iptables -t nat -D PREROUTING <번호1>

# 현재 사설IP로 재등록
HOST_IP=$(hostname -I | awk '{print $1}')
VIP=192.168.122.240   # MetalLB EXTERNAL-IP (doc13 14-4)
...
> SG: 호스트 EC2 인바운드 **80, 443**(0.0.0.0/0) 허용도 같이 확인 — SG 차단은 보통 timeout이라 위 "즉시 refused"와는 증상이 다르지만, 같이 점검할 것.
> `--dport 443`에 `-d $HOST_IP`를 빼면 ghcr.io pull까지 가로채는 별도 버그 → [§20](#20-이미지-pull-실패--tls-certificate-is-valid-for-ingresslocal-not-ghcrio) 참고.
...
### 원인
- doc 13 14-5의 호스트 EC2 DNAT 규칙이 **목적지 IP를 안 가리고 `--dport 443` 전체**를 MetalLB VIP(ingress-nginx)로 보냄.
...
> 예방: doc 13 14-5에 처음부터 `-d $HOST_IP` 포함됨 (이 트러블 이후 반영).
