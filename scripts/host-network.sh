...
#   ip_forward + DNAT(사이트·앱메트릭·kube-state·node_exporter·cadvisor) + FORWARD
#
#   ★ 반드시 "호스트 EC2"(virsh 돌아가는 머신)에서 실행. master VM 아님!
#   ★ VM IP는 virsh로 자동 감지 → 하드코딩·잘못된IP 실수 방지.
#
...
sudo iptables -t nat -A PREROUTING -p tcp --dport 30080 -j DNAT --to-destination $MASTER:30080   # 사이트
...
echo "▶ DNAT 적용 (30080/30400-4/30800/3910x/3808x)"
...
⚠️ SG: 모니터링 SG로부터 30080/30400-30404/30800/38080-38081/39101-39103/9100 인바운드 허용 필요.
DONE
